...
class TestPeerInfo:
    ...
    def test_to_dict_fields(self):
        p = self._make()
        d = p.to_dict()
        for k in ("node_id", "peer_id", "public_key", "addresses",
...
class TestPeerStore:
    ...
    def test_merge_addresses(self):
        s = PeerStore()
        p1 = PeerInfo("NRL1x", "12D3x", "aa" * 32,
                      addresses=["/ip4/1.1.1.1/tcp/7101"])
        p2 = PeerInfo("NRL1x", "12D3x", "aa" * 32,
                      addresses=["/ip4/2.2.2.2/tcp/7101"])
        s.add_or_update(p1)
        s.add_or_update(p2)
...
class TestBootstrapParser:
    ...
    def test_dns4(self):
        ann = _parse_bootstrap_multiaddr("/dns4/bootstrap.neuralis.local/tcp/7101/p2p/NRL1node")
        assert ann is not None
...
async def _make_socket_stream_pair():
    """Create a connected (reader, writer) pair via socketpair. Works on Python 3.10-3.13+."""
    import socket as _socket
    sock_a, sock_b = _socket.socketpair()
    r_a, w_a = await asyncio.open_connection(sock=sock_a)
    r_b, w_b = await asyncio.open_connection(sock=sock_b)
...
class TestHandshakeAndFramedIO:
    ...
    async def test_full_handshake_round_trip(self):
        ...
        (r_a, w_a), (r_b, w_b) = await _make_socket_stream_pair()

        session_a, session_b = await asyncio.gather(
            _perform_handshake_initiator(r_a, w_a, pk_a, node_id_a),
            _perform_handshake_responder(r_b, w_b, pk_b, node_id_b),
        )
    ...
    async def test_framed_send_recv(self):
        ...
        await send_frame(w_a, session_send, message)
        received = await recv_frame(r_b, session_recv)
    ...
    async def test_multiple_frames(self):
        ...
        for m in messages:
            await send_frame(w_a, s_send, m)

        for expected in messages:
            got = await recv_frame(r_b, s_recv)
            assert got == expected
    ...
    async def test_tampered_frame_raises(self):
        ...
        await send_frame(w_a, s_send, b"secret")
        w_a.close()

        # Read the raw encrypted frame, corrupt it, replay via a fresh pair
        raw = await r_b.read(4096)
        tampered = bytearray(raw)
        if len(tampered) > 10:
            tampered[-1] ^= 0xFF

        (r_c, w_c), (r_d, w_d) = await _make_socket_stream_pair()
        w_c.write(bytes(tampered))
        await w_c.drain()

        with pytest.raises(TransportError):
            await recv_frame(r_d, s_recv)
...
class TestMeshHostUnit:
    ...
    async def test_two_nodes_connect_and_exchange(self, tmp_path):
        ...
        assert id_a.node_id in mesh_b.connections or \
               len(mesh_b.connections) > 0, "Node B should have a connection to A"
